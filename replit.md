...
The React application is organized into a clear component hierarchy with pages, reusable components, and UI primitives. Key pages include Home, Services, Packages, About, Contact, and Blog, each designed for specific user journeys.

#### Rebuilt Premium Pages (SEO + Conversion Optimised)
- **About** – Hero with blobs, stats bar, timeline, values, Google Maps, 12-area locality pills, FAQ, @graph JSON-LD
- **Packages** – Animated hero, filter pills, premium package cards, why-choose, FAQ, CTA
- **Tests** – Search + category filter, test cards with Book button, FAQ
- **Contact** – Full rebuild: animated hero, trust bar (6 chips), booking form → WhatsApp, contact info cards, Google Maps embed, trust section, FAQ accordion, final CTA, sticky mobile Call/WhatsApp/Directions bar. Full JSON-LD: ContactPage + MedicalBusiness + LocalBusiness + FAQPage + ContactPoint

#### Global Reveal Animation System
- `App.tsx` runs a global `IntersectionObserver` on all `.reveal*` elements: `threshold: 0`, `rootMargin: "0px"`, immediate viewport check + 100ms/500ms fallbacks
- `use-scroll-reveal.ts` hook also updated to the same threshold so component-level observers match

Components are modular and focused on single responsibilities, such as HeroSection, QuickBooking, ServicesGrid, and testimonials.
...
All major pages (Packages, Blog, About) follow this consistent premium pattern:
1. Animated hero with floating blobs, gradient headline, 3 CTA buttons
2. 6-card colorful stats bar (IntersectionObserver triggered)
3. SEO intro section with keyword-rich paragraph + internal links
4. Content sections with `.reveal` / `.reveal-left` scroll animations
5. FAQ accordion with FAQPage JSON-LD schema
6. Final CTA section with blobs

### Global Scroll Reveal System
`App.tsx` contains `useRevealObserver()` — a global IntersectionObserver that watches all `.reveal`, `.reveal-left`, `.reveal-right`, `.reveal-scale`, `.reveal-fade` elements and adds `.revealed` class when they enter viewport. Fires on every route change. CSS animations defined in `index.css` (revealUp, revealLeft, etc.)

### CSS Animations (index.css)
- blobFloat, heroTextIn, cardEnter, statPop, pillSlide — hero/card entrance
- revealUp/Left/Right/Scale/Fade + .revealed class — scroll animations
- .delay-100 through .delay-700 — staggered animation delays
- test-card-premium shimmer, glowRing — special effects

### Structured Data
SEOHead component (`client/src/components/seo-head.tsx`) injects:
- JSON-LD schema (MedicalBusiness, Organization, AboutPage, FAQPage, BlogPosting, etc.)
- Open Graph + Twitter card meta tags
- Canonical URL

### Key Pages Rebuilt
- `about.tsx` — Full E-E-A-T SEO: hero, stats, journey timeline, trust points, values, Google Maps, areas served, FAQ, CTA. Schemas: AboutPage, Organization, MedicalBusiness, FAQPage
- `packages.tsx` — Premium packages listing with health package cards
- `blog.tsx` — Blog listing with featured post, category filters, premium cards
- `blog-post.tsx` — Individual post with breadcrumbs, content parser, related posts